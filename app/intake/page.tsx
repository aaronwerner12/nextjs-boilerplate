...
export async function POST(req: NextRequest) {
  try {
    ...
    const { orgId, ...formData } = body;

    const id = "intake_" + Date.now().toString(36) + "_" + Math.random().toString(36).substring(2, 7);

    await sql`
      INSERT INTO etf_intake (id, org_id, data, status)
      VALUES (${id}, ${orgId || null}, ${JSON.stringify(formData)}, 'pending')
    `;

    // Look up the specific org's notification email
    try {
      if (orgId) {
        const orgs = await sql`SELECT id, name, notify_email FROM etf_orgs WHERE id = ${orgId} AND notify_email IS NOT NULL AND notify_email != ''`;
        for (const org of orgs) {
          await sendNotificationEmail(formData, org.notify_email, org.name);
        }
      }
    } catch (emailErr) {
...
// GET /api/intake?org_id=xxx — fetch pending intake submissions for this org only
export async function GET(req: NextRequest) {
  try {
    ...
    let rows;
    if (orgId) {
      // Filter by org_id — only show this org's submissions
      rows = await sql`
        SELECT id, data, status, submitted_at, reviewed_at, reviewed_by
        FROM etf_intake
        WHERE status = ${status}
          AND (org_id = ${orgId} OR data->>'orgId' = ${orgId})
        ORDER BY submitted_at DESC
        LIMIT 50
      `;
    } else {
      // No org filter — return all (admin use only)
      rows = await sql`
        SELECT id, data, status, submitted_at, reviewed_at, reviewed_by
        FROM etf_intake
        WHERE status = ${status}
